...
void set_marks(){
    Grade g0;
    fstream file;
    file.open("grade.dat", ios::trunc | ios::in | ios::out | ios::binary);
    if (file)
    {
        char c='c';
        ...
        while(c=='c'||c=='C'){
            ...
            cout<<"Enter date:";
            cin>>date;
            g0.set_data(subjID, subjectName, score, date);
            file.write((char *)&g0, sizeof(g0));
            cout<<"Do you want to add more marks? (c/C for yes, any other key for no): ";
            cin>>c;
        }
    }else{
    ...
    // while (file.read((char *)&g0, sizeof(g0)))
    // {
    //     g0.Log();
    // }
}

Grade get_marks(Grade *grades, int itteration ){
    Grade g0;
    fstream file;
    file.open("grade.dat", ios::in | ios::binary);
    file.seekg(sizeof(g0)*itteration, ios::beg);
    if (file)
    {
        file.read((char *)&g0, sizeof(g0));
        file.close();
        return g0;
    }else{
        cout<<"Can't open file";
